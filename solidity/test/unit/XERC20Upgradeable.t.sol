...
abstract contract Base is Test {
  ...
  address internal _admin = vm.addr(4);

  uint256 internal constant MAX_LIMIT = type(uint256).max / 2;

  address internal _implementation;
  XERC20Upgradeable internal _xerc20;
  ...
  function setUp() public virtual {
    _implementation = address(new XERC20Upgradeable());

    bytes memory initialize = abi.encodeWithSelector(XERC20Upgradeable.initialize.selector, 'Test', 'TST', _owner, 0);

    vm.prank(_owner);
    _proxy = new TransparentUpgradeableProxy(_implementation, _admin, initialize);
    _xerc20 = XERC20Upgradeable(address(_proxy));
  }
}
...
contract UnitDeploy is Base {
  function testDeployment(uint256 _initialSupply) public {
    bytes memory initialize =
      abi.encodeWithSelector(XERC20Upgradeable.initialize.selector, 'Test', 'TST', _owner, _initialSupply);
    ...
    assertEq(_xerc20.owner(), _owner);
    assertEq(_xerc20.balanceOf(_owner), _initialSupply);
  }
}
