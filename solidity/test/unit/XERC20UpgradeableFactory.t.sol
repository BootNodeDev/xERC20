...
import {XERC20UpgradeableFactory} from '../../contracts/XERC20UpgradeableFactory.sol';
...
contract XERC20FactoryForTest is XERC20UpgradeableFactory {
...
contract UnitDeploy is Base {
  function testDeployment(uint256 _initialSupply) public {
    ...
    vm.prank(_owner);
    (address _xerc20, address _admin) =
      _xerc20Factory.deployXERC20Upgradeable('Test', 'TST', _limits, _limits, _minters, _initialSupply, address(0));
    assertEq(XERC20Upgradeable(_xerc20).name(), 'Test');
    assertEq(XERC20Upgradeable(_xerc20).owner(), _owner);
    assertEq(XERC20Upgradeable(_xerc20).balanceOf(_owner), _initialSupply);
    assertEq(ProxyAdmin(_admin).owner(), _owner);
  }

  function testDeploymentDifferentOwner(uint256 _initialSupply) public {
    uint256[] memory _limits = new uint256[](0);
    address[] memory _minters = new address[](0);
    ...
    vm.prank(_owner);
    (address _xerc20, address _admin) =
      _xerc20Factory.deployXERC20Upgradeable('Test', 'TST', _limits, _limits, _minters, _initialSupply, differentOwner);
    assertEq(XERC20Upgradeable(_xerc20).name(), 'Test');
    assertEq(XERC20Upgradeable(_xerc20).owner(), differentOwner);
    assertEq(XERC20Upgradeable(_xerc20).balanceOf(_owner), _initialSupply);
    assertEq(ProxyAdmin(_admin).owner(), differentOwner);
  }
  ...
  function testRevertsWhenAddressIsTaken() public {
    ...
    vm.prank(_owner);
    _xerc20Factory.deployXERC20Upgradeable('Test', 'TST', _limits, _limits, _minters, 0, address(0));
  ...
  function testNotParallelArraysRevert() public {
    ...
    _xerc20Factory.deployXERC20Upgradeable('Test', 'TST', _minterLimits, _empty, _minters, 0, address(0));

    vm.expectRevert(IXERC20Factory.IXERC20Factory_InvalidLength.selector);
    _xerc20Factory.deployXERC20Upgradeable('Test', 'TST', _empty, _burnerLimits, _minters, 0, address(0));
  }
  ...
  function testDeployEmitsEvent() public {
    ...
    emit XERC20Deployed(_token, address(0));
    vm.prank(_owner);
    _xerc20Factory.deployXERC20Upgradeable('Test', 'TST', _limits, _limits, _minters, 0, address(0));
  }
