...
contract CommonE2EBase is Test {
  ...
  function setUp() public virtual {
    ...
    address _token =
      _xerc20Factory.deployXERC20(_dai.name(), _dai.symbol(), _minterLimits, _burnerLimits, _bridges, 0, address(0));
...
contract CommonE2EBaseUpgrade is Test {
  uint256 internal constant _FORK_BLOCK = 18_213_198;
  uint256 internal _userPrivateKey = 0x1234;
  address internal _user = vm.addr(_userPrivateKey);
  address internal _owner = vm.addr(200);
  ...
  XERC20Lockbox internal _lockbox;
  XERC20UpgradeableFactory internal _xerc20Factory;
  ...
  function setUp() public virtual {
    ...
    vm.startPrank(_owner);
    _xerc20Factory = new XERC20UpgradeableFactory();
    (address _token,) = _xerc20Factory.deployXERC20Upgradeable(
      _dai.name(), _dai.symbol(), _minterLimits, _burnerLimits, _bridges, 0, address(0)
    );
    address payable _lock = _xerc20Factory.deployLockbox(_token, address(_dai), false);

    _xerc20 = XERC20Upgradeable(_token);
    _lockbox = XERC20Lockbox(_lock);
    vm.stopPrank();
  }
}
